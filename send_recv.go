...
func (t *TraceRoute) SendIPv4UDP() {
	...
	sport := uint16(40000 + rand.Int31n(10000))
	key := GetHash(t.netSrcAddr.To4(), t.netDstAddr.To4(), sport, dport)
	db := NewStatsDB(key)
	...
	for {
		...
		if (icmpType == 11 || (icmpType == 3 && buf[1] == 3)) && (n >= 36) { //TTL Exceeded or Port Unreachable
			...
			if dstip.Equal(t.netDstAddr) { // && dstPort == t.dstPort {
				key := GetHash(srcip, dstip, srcPort, dstPort)
